...
while (ens_t < fcst_nt):
   ...
   for f, file in enumerate(summary_files_temp):
      ...
      if ((file[-28:-25] == 'ENS') and (file[-24:-22] == str_ens_t)):
         ...
         if (ens_t >= 3): 
            cmd = "/home/louis.wicker/anaconda2/envs/wof-test/bin/python news_e_paintball_ltc.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, (ens_t-3), fcst_nt)
...
cmd = "/home/louis.wicker/anaconda2/envs/wof-test/bin/python news_e_paintball_ltc.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, (ens_t-2), fcst_nt)
pool.apply_async(run_script, (cmd,))

cmd = "/home/louis.wicker/anaconda2/envs/wof-test/bin/python news_e_paintball_ltc.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, (ens_t-1), fcst_nt)
pool.apply_async(run_script, (cmd,))

cmd = "/home/louis.wicker/anaconda2/envs/wof-test/bin/python news_e_paintball_ltc.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, (ens_t), fcst_nt)
pool.apply_async(run_script, (cmd,))
